...
//判断一个字符串是否为utf8编码
//////////////////////////////////////////////////////////////////////////
const char *_pinyin_table[20902] = {
...
py_info_t _infos[20902];

void init_pinyin () {
    int hanzi_num = sizeof(_pinyin_table) / sizeof(const char*);
    for (int i = 0; i < hanzi_num; ++i) {
        py_info_t& info = _infos[i];
        int count = 1;
        int len = 0;
        const char* s = _pinyin_table[i];
        while (true) {
            if ('\0' == s[len]) {
                break; 
            }  
        ...
        memcpy(s1, s, len + 1);
        info.num = count;
        info.ptrs = (char**)malloc(sizeof(char*) * count);
        count = 0;
        while (true) {
            char* p = strchr(s1, '|'); 
            if (p) {
...
const py_info_t* pinyin_utf8(char* inbuf) {
    if (3 != strlen(inbuf)) {
        return NULL; 
    }
    int uni;

    if ((inbuf[0] & 0xF0) != 0xE0){    //三个字符，汉字、日语等亚洲字符
        return NULL;
    }
    uni = (((int)(inbuf[0] & 0x0F)) << 12)
            | (((int)(inbuf[1] & 0x3F)) << 6)
            | (inbuf[2] & 0x3F);
    if (uni>19967&&uni<40870) {
        return _infos + (uni - 19968);
    }
    return NULL;
}
